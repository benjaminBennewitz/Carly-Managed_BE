...
class SiteContextTests(SimpleTestCase):
    ...
    def test_request_host_selects_matching_site_context(self) -> None:
        ...
        self.assertEqual(context.frontend_url, "https://carly-managed-demo.design-code-repeat.de")
        self.assertEqual(
            context.from_email,
    ...
    def test_default_https_port_keeps_matching_host(self) -> None:
        ...
        request = RequestFactory().get(
            "/api/v1/auth/csrf/",
            secure=True,
            HTTP_HOST="carly-managed-demo.b2folio.de",
        )
        request.META["HTTP_HOST"] = "carly-managed-demo.b2folio.de:443"

        context = resolve_site_context(request)

        self.assertEqual(context.frontend_url, "https://carly-managed-demo.b2folio.de")
